...
OUT=compare_eqTFAN_mesh_plane_self
echo $OUT
$CMD compare --mode eqTFAN --inputModelA ${DATA}/plane.obj --inputModelB ${DATA}/plane.obj > ${TMP}/${OUT}.txt 2>&1
fileHasString ${TMP}/${OUT}.txt "meshes are equal" 1

# vertex 2 and 4 are swaped and some face indices are shifted
OUT=compare_eqTFAN_mesh_plane_shifted
echo $OUT
$CMD compare --mode eqTFAN --inputModelA ${DATA}/plane.obj --inputModelB ${DATA}/plane_shifted.obj > ${TMP}/${OUT}.txt 2>&1
fileHasString ${TMP}/${OUT}.txt "meshes are equal" 1

#meshes non-manifold
OUT=compare_eqTFAN_mesh_test_non-manifold
echo $OUT
...
echo $OUT
$CMD compare --mode eqTFAN --inputModelA ${DATA}/test_inputA.obj --inputModelB ${DATA}/test_inputB.obj > ${TMP}/${OUT}.txt 2>&1
fileHasString ${TMP}/${OUT}.txt "meshes are not equal" 1

# vertex 2 and 4 are swaped and some faces indices enumerated in revert order, disable early return
OUT=compare_eqTFAN_mesh_plane_reorder
echo $OUT
$CMD compare --mode eqTFAN --eqTFAN_earlyReturn=false --inputModelA ${DATA}/plane.obj --inputModelB ${DATA}/plane_reorder.obj > ${TMP}/${OUT}.txt 2>&1
...
OUT=compare_eqTFAN_mesh_plane_sphere
echo $OUT
$CMD compare --mode eqTFAN --inputModelA ${DATA}/plane.obj --inputModelB ${DATA}/sphere.obj > ${TMP}/${OUT}.txt 2>&1
fileHasString ${TMP}/${OUT}.txt "meshes are not equal" 1

# compare point clouds
OUT=compare_eqTFAN_pc_plane_self_eps_0
echo $OUT
$CMD sample --mode sdiv --inputModel ${DATA}/plane.obj --inputMap ${DATA}/plane.png --outputModel ID:sampled END \
	compare --mode eqTFAN --inputModelA ID:sampled --inputModelB ID:sampled \
...
OUT=compare_eqTFAN_pc_plane_eps_0_01
echo $OUT
$CMD sample --mode sdiv --inputModel ${DATA}/plane.obj --inputMap ${DATA}/plane.png --outputModel ID:sampled END \
	compare --mode eqTFAN --eqTFAN_epsilon 0.01 --inputModelA ID:sampled --inputModelB ID:sampled \
	--outputModelA ${TMP}/${OUT}_A.ply --outputModelB ${TMP}/${OUT}_B.ply > ${TMP}/${OUT}.txt 2>&1
fileHasString ${TMP}/${OUT}.txt "model vertices are equals" 1

OUT=compare_eqTFAN_pc_plane_sphere_eps_0
echo $OUT
$CMD sample --mode sdiv --inputModel ${DATA}/plane.obj --outputModel ID:plane END \
	...
	compare --mode eqTFAN --inputModelA ID:plane --inputModelB ID:sphere > ${TMP}/${OUT}.txt 2>&1
fileHasString ${TMP}/${OUT}.txt "model vertices are not equals" 1

OUT=compare_eqTFAN_pc_plane_sphere_eps_0_01
echo $OUT
$CMD sample --mode sdiv --inputModel ${DATA}/plane.obj --outputModel ID:plane END \
	sample --mode sdiv --inputModel ${DATA}/sphere.obj --outputModel ID:sphere END \
	compare --mode eqTFAN --eqTFAN_epsilon 0.01 --inputModelA ID:plane --inputModelB ID:sphere > ${TMP}/${OUT}.txt 2>&1
fileHasString ${TMP}/${OUT}.txt "model vertices are not equals" 1
...
then
	...
	OUT=compare_eqTFAN_basket_self
	echo $OUT
	$CMD compare --mode eqTFAN \
	...
	OUT=compare_eqTFAN_basket_yuv420
	echo $OUT
	$CMD  \
		compare --mode eqTFAN  \
	...
	OUT=compare_eqTFAN_basket_gbrp444
	echo $OUT
	$CMD  \
		compare --mode eqTFAN  \
		--inputModelA ${DATA}/basketball_player_fr0001_qp12_qt12_dequant.obj \
